...
export function ChatList({ name, text, time, num, Imgsrc, isOnline, isAction, sent, handleClick, id }: ChatListProps) {
  const { colors } = useTheme();
...
export default function Addchat({ handleClick }: AddchatProps) {
  const { colors } = useTheme();
  ...
  if (isError) {
    return (
      <div className="flex flex-col h-full w-full items-center justify-center"
        style={{ background: colors.background }}>
        <p style={{ color: colors.textSecondary }}>Error: {(error as Error).message}</p>
      </div>
    );
  }
            ...
            friends.data.map((users) => {
              ...
              return (
                <ChatList
                  ...
                  time={FormatHometime(lastDm?.data?.last_messageTime) || ""}
